    ...
    <section className="relative pt-32 pb-24 md:pt-40 md:pb-32 overflow-hidden">
      {video && (
        <>
          ...
          <div className="absolute inset-0 z-0" style={{ background: "linear-gradient(to top, var(--bg-tint-97), var(--bg-tint-97) 10%, rgba(17,17,17,0.80) 50%, rgba(17,17,17,0.60))" }} />
        </>
      )}
      {!video && image && (
        <>
      ...
      {!hasMedia && (
        <div className="absolute inset-0 z-0 pointer-events-none" style={{ background: "linear-gradient(to bottom, transparent 50%, var(--tint-8))" }} />
      )}
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        <div className="flex items-center gap-4 mb-6">
          <span className="w-8 h-[2px]" style={{ backgroundColor: "var(--link-color, #E63B2E)" }} />
          <span className="font-['Space_Mono'] uppercase tracking-widest text-sm" style={{ color: "#F0473D" }}>{label}</span>
        </div>
