...
export function BlogSection({ posts: postsProp }: { posts?: BlogPost[] }) {
  ...
  return (
    <section id="blog" className="py-24 md:py-32 relative border-t" style={{ borderColor: "var(--border-color)" }}>
          ...
          {posts.map((post, i) => (
            ...
            >
              <Link
                ...
                className="group flex flex-col border transition-colors overflow-hidden w-full"
                style={{ borderColor: "var(--border-color)", backgroundColor: "var(--bg-secondary, #1A1A1A)" }}
              >
                {post.cover_image && (
