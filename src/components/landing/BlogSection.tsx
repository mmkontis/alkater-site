...
export function BlogSection({ posts: postsProp }: { posts?: BlogPost[] }) {
  ...
  return (
    <section id="blog" className="py-24 md:py-32 relative border-t overflow-hidden" style={{ borderColor: "var(--border-color)", backgroundColor: "var(--bg-tint-88)" }}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 60% 50% at 90% 0%, var(--tint-8), transparent)" }} />
          ...
          {posts.map((post, i) => (
            ...
            >
              <Link
                ...
                style={{ borderColor: "var(--blog-card-border)", backgroundColor: "var(--blog-card-bg)" }}
              >
                {post.cover_image && (
                  <div className="relative aspect-[16/9] overflow-hidden">
                    ...
                    <div className="absolute inset-0 opacity-30 group-hover:opacity-0 transition-opacity duration-500" style={{ background: "linear-gradient(135deg, var(--tint-25), transparent)" }} />
                  </div>
                )}
